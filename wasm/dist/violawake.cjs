...
class OWWBackbone {
    ...
    static async create(melspecModelUrl, embeddingModelUrl, ortOptions) {
        ...
        const [mel, emb] = await Promise.all([
            ort__namespace.InferenceSession.create(melspecModelUrl, opts),
            ort__namespace.InferenceSession.create(embeddingModelUrl, opts),
        ]);
    ...
    async _predictMelspectrogram(pcmI16) {
        ...
        const tensor = new ort__namespace.Tensor("float32", f32, [1, f32.length]);
    ...
    async _predictEmbedding(melWindow) {
        ...
        const tensor = new ort__namespace.Tensor("float32", input, [1, rows, cols, 1]);
...
class WakeDetector {
    ...
    async load() {
        ...
        this.classifierSession = await ort__namespace.InferenceSession.create(this.classifierModelUrl, this.ortOptions);
        ...
        if (shape?.length === 3) {
            ...
            throw new Error(`Classifier model has unsupported input shape ${JSON.stringify(shape)}; expected ` +
    ...
    async _runTemporalClassifier() {
        ...
        const tensor = new ort__namespace.Tensor("float32", flat, [1, this.temporalSeqLen, EMBEDDING_DIM]);
    ...
    async _runMlpClassifier(embedding) {
        ...
        const tensor = new ort__namespace.Tensor("float32", embedding.slice(), [1, EMBEDDING_DIM]);
        const feeds = { [this.classifierInputName]: tensor };
        const results = await this.classifierSession.run(feeds);
        const output = results[this.classifierSession.outputNames[0]];
        return output.data[0];
    ...
    _getClassifierOutputShape() {
        const metadata = this.classifierSession?.outputMetadata;
        if (!metadata) {
            return null;
        }
        const outputName = this.classifierSession.outputNames[0];
        const outputMeta = Array.isArray(metadata)
            ? metadata.find((entry) => entry?.name === outputName) ?? metadata[0]
            : metadata[outputName];
        const shape = outputMeta?.dimensions ?? outputMeta?.shape;
        return Array.isArray(shape) ? shape : null;
    }
    _validateClassifierInputShape(shape) {
        const lastDim = shape[shape.length - 1];
        if (lastDim !== EMBEDDING_DIM) {
            throw new Error(`Classifier model input must end with ${EMBEDDING_DIM} OWW embedding dimensions; ` +
    ...
    _validateClassifierOutputShape() {
        const shape = this._getClassifierOutputShape();
        if (shape === null) {
            return;
        }
        const concreteDims = shape.filter((dim) => typeof dim === "number");
        const elementCount = concreteDims.reduce((acc, dim) => acc * dim, 1);
        if (concreteDims.length > 0 && elementCount !== 1) {
            throw new Error(`Classifier model output must be a single scalar score; got shape ${JSON.stringify(shape)}.`);
        }
    }
    _validateAudioFrame(audioBuffer) {
        ...
        if (audioBuffer.length !== FRAME_SAMPLES) {
            throw new RangeError(`audioBuffer must contain exactly ${FRAME_SAMPLES} samples ` +
        ...
        for (let i = 0; i < audioBuffer.length; i++) {
            ...
            if (sample < -1 || sample > 1.0) {
                throw new RangeError(`audioBuffer sample ${i} must be normalized to [-1, 1]; got ${sample}.`);
            }
        }
    }
...
exports.MEL_FRAMES_PER_EMBEDDING = MEL_FRAMES_PER_EMBEDDING;
exports.MEL_STRIDE = MEL_STRIDE;
exports.OWWBackbone = OWWBackbone;
exports.OWW_CHUNK_SAMPLES = OWW_CHUNK_SAMPLES;
exports.SAMPLE_RATE = SAMPLE_RATE;
exports.WakeDetector = WakeDetector;
//# sourceMappingURL=violawake.cjs.map
