...
import { z } from "zod";
import { sendEmailAudited, OutboundPausedError } from "./email-sender";
...
app.delete("/api/v1/mailboxes/:mailboxId", async (c) => {
	...
	// 1. Purge DO data (emails, attachments, folders) and collect R2 keys.
	const stub = c.env.MAILBOX.get(c.env.MAILBOX.idFromName(mailboxId));
	let r2Keys: string[] = [];
	try {
		const result = await (stub as any).purgeAllData() as { r2Keys: string[] };
		r2Keys = result.r2Keys ?? [];
	} catch (e) {
		console.error("purgeAllData failed during mailbox delete:", (e as Error).message);
		// Non-fatal: the settings object is deleted below regardless.
		// The DO will be orphaned with data in it — log for manual cleanup.
	}

	// 2. Delete R2 attachment blobs (batched — R2 delete accepts up to 1000 keys).
	const BATCH = 1000;
	for (let i = 0; i < r2Keys.length; i += BATCH) {
		await c.env.BUCKET.delete(r2Keys.slice(i, i + BATCH));
	}

	// 3. Delete mailbox settings object.
...
app.post("/api/v1/mailboxes/:mailboxId/emails", async (c: AppContext) => {
	...
	c.executionCtx.waitUntil(
		sendEmailAudited(c.env.BUCKET, c.env.EMAIL, mailboxId, {
			to, cc, bcc, from, subject, html, text,
			attachments: attachments?.map((att) => ({ content: att.content, filename: att.filename, type: att.type, disposition: att.disposition || "attachment", contentId: att.contentId })),
			...(in_reply_to ? { headers: buildThreadingHeaders(in_reply_to, references || []) } : {}),
		}, "http").catch((e) => console.error("Deferred email delivery failed:", (e as Error).message)),
	);
...
/**
 * Classify whether an inbound email is automated (bounce, vacation reply,
 * auto-responder, mailing list, etc.) so the worker never auto-acts on it.
 *
 * Rules per RFC 3834 §3.1 and common MTА conventions:
 *
 * 1. Auto-Submitted header present and value is not "no"     → automated
 * 2. X-Auto-Response-Suppress header present (any value)    → automated
 * 3. Precedence: bulk | list | junk                         → automated
 * 4. List-Id or List-Unsubscribe header present             → mailing list
 * 5. Content-Type: multipart/report (delivery status, etc.) → NDR bounce
 * 6. Sender address is MAILER-DAEMON@ or postmaster@        → bounce
 * 7. Subject matches common bounce/vacation patterns         → automated
 *
 * Returns a short reason string when automated, null when it looks like
 * genuine human mail.
 *
 * This classification only suppresses the auto-draft trigger; the email
 * is always stored in the inbox regardless.
 */
function classifyAutoMail(
	parsedEmail: Awaited<ReturnType<InstanceType<typeof PostalMime>["parse"]>>,
): string | null {
	const headerMap = new Map<string, string>();
	for (const h of parsedEmail.headers) {
		headerMap.set(h.key.toLowerCase(), h.value.toLowerCase());
	}

	// 1. Auto-Submitted (RFC 3834): any value except "no" means automated.
	...
	// 2. X-Auto-Response-Suppress (Exchange/Outlook convention).
	if (headerMap.has("x-auto-response-suppress")) {
		return "X-Auto-Response-Suppress header present";
	}

	// 3. Precedence: bulk | list | junk.
	const precedence = headerMap.get("precedence");
	if (precedence && /\b(bulk|list|junk)\b/i.test(precedence)) {
		return `Precedence: ${precedence}`;
	}

	// 4. Mailing-list infrastructure headers.
	if (headerMap.has("list-id") || headerMap.has("list-unsubscribe")) {
		return "mailing-list header (List-Id / List-Unsubscribe)";
	}

	// 5. Content-Type: multipart/report  → DSN / MDN bounce.
	const ct = headerMap.get("content-type") ?? "";
	if (/^multipart\/report\b/i.test(ct)) {
		return `multipart/report content-type (${ct.slice(0, 60)})`;
	}

	// 6. Sender is MAILER-DAEMON or postmaster — RFC 5321 bounce originator.
	const fromAddr = (parsedEmail.from?.address || "").toLowerCase();
	if (/^(mailer-daemon|postmaster)@/i.test(fromAddr) || fromAddr === "mailer-daemon") {
		return `bounce originator address: ${fromAddr}`;
	}

	// 7. Subject patterns that reliably indicate automated messages.
	const subject = (parsedEmail.subject || "").toLowerCase();
	const AUTOMATED_SUBJECT_RE =
		/^(undeliverable|undelivered mail|delivery (status notification|failure|report)|mail delivery failed|returned mail|bounce notification|auto(matic(ally)?)?[:\s]|out of office|vacation|away from (the )?office|on holiday)/i;
	if (AUTOMATED_SUBJECT_RE.test(parsedEmail.subject || "")) {
		return `automated-looking subject: "${parsedEmail.subject?.slice(0, 80)}"`;
	}
	void subject; // subject used indirectly above via parsedEmail.subject
...
async function receiveEmail(event: { raw: ReadableStream; rawSize: number }, env: Env, ctx: ExecutionContext) {
	...
	// Capture mail to ANY address at our domain into the PRIMARY support mailbox
	// (EMAIL_ADDRESSES[0]), so a Cloudflare catch-all can route every address here
	// and nothing is dropped for not being pre-configured. A recipient that DOES
	// match a configured address still lands in its own mailbox; everything else
	// (support@, sales@, typos, …) is funneled into the primary inbox. The real
	// original recipients are preserved on the stored message (`recipient` below),
	// so the operator still sees who each message was actually sent to.
	let mailboxId: string | undefined;
	if (allowedAddresses.length > 0) {
		mailboxId = allRecipients.find((addr) => allowedAddresses.includes(addr)) ?? allowedAddresses[0];
	} else {
	...
	const messageId = crypto.randomUUID();
	// Auto-provision the primary mailbox's settings object if missing, so capture
	// never silently drops (the catch-all funnels arbitrary addresses here).
	if (!(await env.BUCKET.head(`mailboxes/${mailboxId}.json`))) {
		const defaults = { fromName: mailboxId, forwarding: { enabled: false, email: "" }, signature: { enabled: false, text: "" }, autoReply: { enabled: false, subject: "", message: "" } };
		await env.BUCKET.put(`mailboxes/${mailboxId}.json`, JSON.stringify(defaults), { httpMetadata: { contentType: "application/json" } });
	}
	...
	// Suppress auto-draft for bounces, vacation notices, auto-responders, and
	// mailing-list mail (RFC 3834 + common MTA conventions).  The email is
	// already stored in the inbox above — only the agent trigger is skipped.
	const autoMailReason = classifyAutoMail(parsedEmail);
	if (autoMailReason) {
		console.log(`Auto-draft suppressed for message ${messageId}: ${autoMailReason}`);
		return;
	}
