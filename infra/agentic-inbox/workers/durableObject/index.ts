...
export class MailboxDO extends DurableObject<Env> {
	...
	// ── Full mailbox purge (GDPR / account erasure) ────────────────────

	/**
	 * Collect all R2 attachment key paths for this mailbox and wipe the
	 * SQLite storage completely so no email data remains in the DO.
	 *
	 * The caller (DELETE /api/v1/mailboxes/:mailboxId) is responsible for:
	 *   1. Calling this method to get the R2 keys + wipe the DO.
	 *   2. Deleting the returned R2 keys from BUCKET.
	 *   3. Deleting the mailbox settings object (mailboxes/<id>.json) from BUCKET.
	 *
	 * Returns the list of R2 key paths that must be deleted from BUCKET.
	 */
	async purgeAllData(): Promise<{ r2Keys: string[] }> {
		// Collect all attachment rows so we can build R2 keys before deletion.
		const allAttachments = [
			...this.ctx.storage.sql.exec(
				`SELECT email_id, id, filename FROM attachments`,
			),
		] as Array<{ email_id: string; id: string; filename: string }>;

		const r2Keys = allAttachments.map(
			(a) => `attachments/${a.email_id}/${a.id}/${a.filename}`,
		);

		// Wipe all data: DELETE CASCADE from folders removes emails + attachments.
		// Reset d1_migrations so any future migration run starts clean.
		...
		this.ctx.storage.sql.exec(`DELETE FROM d1_migrations`);
